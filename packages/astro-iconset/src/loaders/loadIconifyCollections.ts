...
export default async function loadIconifyCollections({
  ...
  const collections = possibleCollections.reduce<AstroIconCollectionMap>(
    (acc, [name, collection]) => {
      if (!collection) {
        console.error(
          `[astro-iconset] "${name}" does not appear to be a valid iconify collection! Did you install the "@iconify-json/${name}" dependency?`,
        );
        return acc;
      }

      const requestedIcons = Array.from(new Set(resolved[name]));
      ...
      if (!reducedCollection) {
        console.error(
          `[astro-iconset] "${name}" failed to load the specified icons!`,
        );
        return acc;
      } else if (
        ...
        console.error(
          `[astro-iconset] "${name}" failed to load at least one of the specified icons! Verify the icon names are included in the icon collection.`,
        );
      }
